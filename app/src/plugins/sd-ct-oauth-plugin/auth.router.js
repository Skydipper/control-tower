...
module.exports = (plugin, connection, generalConfig) => {
    ...
    const getSpecificUser = (ctx, type) => {
        if (ctx.state[type]) return ctx.state[type];
        if (ctx.req[type]) return ctx.req[type];

        const user = getUser(ctx);
        if (user.id === type) return user;
